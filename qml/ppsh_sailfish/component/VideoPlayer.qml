...
Rectangle {
	...
	Connections{
		target: _PLAYER;
		onError: {
			if(root.__playerEnabled) controller._ShowMessage("[%1]: %2 -> %3(%4)".arg(qsTr("ERROR")).arg(pid).arg(err_no).arg(errstr));
		}
	}
	...
	QtObject{
		...
		function _LoadSource(url, pos)
		{
			...
			if(st.isDashFormat)
			{
				console.log("with audio source -> " + st._GetDashAudio());
				controller._ShowMessage(qsTr("Dash format(extras audio maybe not synchronize with video)..."));
				d._Seek();
				v._Load(url, pos, st._GetDashAudio());
				// _PlayExternally(url); // sailfish support decode
			}
			else
			{
				d._Seek();
