...
void QDeclarativeAudio::setSource(const QUrl &url)
{
    ...
    m_source = url;
#ifdef _KARIN_MM_EXTENSIONS
    m_content = m_source.isEmpty() ? QMediaContent() : MediaContent(m_source);
#else
...
#ifdef _KARIN_MM_EXTENSIONS
    m_metaData.reset(new QDeclarativeMediaMetaData(dynamic_cast<QMetaDataReaderControl *>(m_player->service()->requestControl(QMetaDataReaderControl_iid))));
#else
...
#ifdef _KARIN_MM_EXTENSIONS
QVariantList QDeclarativeAudio::RequestHeaders() const
{
	return tRequestHeaders;
}

void QDeclarativeAudio::SetRequestHeaders(const QVariantList &headers)
{
	if(tRequestHeaders != headers)
	{
		...
		Q_FOREACH(const QVariant &v, headers)
		{
			...
			AddHeader(QByteArray().append(key), QByteArray().append(value));
		}
		emit requestHeadersChanged();
	}
}

bool QDeclarativeAudio::HeadersEnabled() const
{
	return bUsingHeaders;
}

void QDeclarativeAudio::SetHeadersEnabled(bool b)
{
	if(bUsingHeaders != b)
	{
		bUsingHeaders = b;
...
void QDeclarativeAudio::SetHeaders(const QMap<QByteArray, QByteArray> &headers)
{
	if(tHeaders != headers)
	{
		tHeaders = headers;
...
QMap<QByteArray, QByteArray> QDeclarativeAudio::Headers() const
{
	return tHeaders;
}

void QDeclarativeAudio::ClearHeaders()
{
	if(!tHeaders.isEmpty())
	{
		tHeaders.clear();
...
QByteArray QDeclarativeAudio::GetHeader(const QByteArray &name) const
{
	return tHeaders.value(name);
}

void QDeclarativeAudio::SetHeader(const QByteArray &name, const QByteArray &value)
{
...
QDeclarativeAudio * QDeclarativeAudio::RemoveHeader(const QByteArray &name)
{
...
QDeclarativeAudio * QDeclarativeAudio::AddHeader(const QByteArray &name, const QByteArray &value)
{
	if(!tHeaders.contains(name))
	{
...
QMediaContent QDeclarativeAudio::MediaContent(const QUrl &url)
{
	if(!bUsingHeaders)
...
#endif
	return QMediaContent(req);
}

bool QDeclarativeAudio::CheckSource(const QUrl &url) const
{
	if(url.isEmpty())
		return false;
