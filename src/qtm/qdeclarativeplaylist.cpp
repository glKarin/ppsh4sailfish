...
#ifdef _KARIN_MM_EXTENSIONS
QVariantList QDeclarativePlaylist::RequestHeaders() const
{
	return tRequestHeaders;
}

void QDeclarativePlaylist::SetRequestHeaders(const QVariantList &headers)
{
...
bool QDeclarativePlaylist::HeadersEnabled() const
{
	return bUsingHeaders;
}

void QDeclarativePlaylist::SetHeadersEnabled(bool b)
{
	if(bUsingHeaders != b)
	{
		bUsingHeaders = b;
...
void QDeclarativePlaylist::SetHeaders(const QMap<QByteArray, QByteArray> &headers)
{
	if(tHeaders != headers)
	{
		tHeaders = headers;
...
QMap<QByteArray, QByteArray> QDeclarativePlaylist::Headers() const
{
	return tHeaders;
}

void QDeclarativePlaylist::ClearHeaders()
{
	if(!tHeaders.isEmpty())
	{
		tHeaders.clear();
...
QByteArray QDeclarativePlaylist::GetHeader(const QByteArray &name) const
{
	return tHeaders.value(name);
}

void QDeclarativePlaylist::SetHeader(const QByteArray &name, const QByteArray &value)
{
...
QDeclarativePlaylist * QDeclarativePlaylist::RemoveHeader(const QByteArray &name)
{
...
QDeclarativePlaylist * QDeclarativePlaylist::AddHeader(const QByteArray &name, const QByteArray &value)
{
	if(!tHeaders.contains(name))
	{
...
QMediaContent QDeclarativePlaylist::MediaContent(const QUrl &url)
{
	if(!bUsingHeaders)
...
bool QDeclarativePlaylist::CheckSource(const QUrl &url) const
{
	if(url.isEmpty())
		return false;
