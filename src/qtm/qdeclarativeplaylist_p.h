...
private:
		QMediaContent MediaContent(const QUrl &url);
		bool CheckSource(const QUrl &url) const;
		...
		QDeclarativePlaylist * RemoveHeader(const QByteArray &name);
		QDeclarativePlaylist * AddHeader(const QByteArray &name, const QByteArray &value);
